...
if jq -e '.mcpServers | has("linear-server")' "$TARGET/.claude.json" >/dev/null 2>&1; then
  printf '    linear-server already configured\n'
else
  run claude mcp add --scope user --transport http \
    linear-server https://mcp.linear.app/mcp || warn "could not add linear-server"
fi
