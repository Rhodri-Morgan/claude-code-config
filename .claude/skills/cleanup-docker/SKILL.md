...
- **`docker rm` and `docker volume rm` prompt** under the permission rules in this config (`Bash(* rm *)` is in `ask`). The prune-with-filter forms above do the same job unattended, which is why they are used throughout.
